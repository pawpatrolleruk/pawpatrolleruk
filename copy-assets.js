...
function createDefaultReviewImage() {
  ...
  try {
    ...
    if (fs.existsSync(CONFIG.defaultImage.templatePath)) {
      fs.copyFileSync(
        CONFIG.defaultImage.templatePath,
        CONFIG.defaultImage.path
      );
...
function getExpectedImagesFromReviews() {
  try {
    const reviewsJsonPath = path.join(
    ...
    console.log(
      `Found ${expectedImages.length} images referenced in reviews.json`
    );
  ...
  files.forEach((file) => {
    ...
    try {
      ...
      if (stats.isFile()) {
        ...
        copyFiles(sourcePath, destPath, { isImage, expectedFiles });
      }
    } catch (error) {
...
function validateCopiedImages(
  ...
  const missingImages = expectedFiles.filter(
    (image) => !copiedFiles.includes(image)
  );
  ...
  console.warn(
    `Warning: ${missingImages.length} images referenced in reviews.json were not found`
  );
  ...
  missingImages.forEach((image) => {
    ...
    if (similarFiles.length > 0) {
      ...
      try {
        ...
        console.log(
          `    Auto-corrected: Copied ${similarFiles[0]} as ${image}`
        );
      } catch (error) {
...
async function copyAllAssets() {
  ...
  try {
    ...
    Object.values(CONFIG.directories.destination).forEach(
    ...
    console.log(
      `Asset copy process completed in ${(endTime - startTime) / 1000} seconds`
    );
  } catch (error) {
